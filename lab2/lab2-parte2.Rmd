...
Com o nosso modelo melhorado, vemos que o R² ajustado aumentou, temos agora que nosso modelo consegue explicar ~65% da variação dos dados. 
...
Nos gráficos acima podemos ver o relacionamento entre as variáveis. No primeiro modelo proposto, a variável LP1 foi removida, vamos agora analisar se essa variável possui alguma influência significativa, quando está presente apenas as disciplinas do primeiro período.
...
Nos gráficos acima podemos ver o relacionamento entre as variáveis. A disciplina de IC (Introdução a Computação) é a que possui a maior correlação com a variável resposta. 
No primeiro modelo proposto, a variável LP1 foi removida, vamos agora analisar se essa variável possui alguma influência significativa, quando está presente apenas as disciplinas do primeiro período.
...
```{r fig.width=12, fig.height=10}
lm.p1 <- lm(primeiro.periodo$CRA ~ . -LP1, primeiro.periodo)
summary(lm.p1)
```
