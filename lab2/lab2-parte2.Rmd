...
Analisando a matriz de correlação das variáveis, observamos que grande parte possue uma correlaçao até 0.5 ou inferior, baseado nisso assumimos um treshhold de 0.65, ou seja, acima desse valor a correlação entre as variaveis e considerada alta. 
...
```{r}
lm.p1p2.melhorado <-lm(CRA ~ . -LP1 -Grafos -P2, p1.p2)
...
As variaveis retiradas foram LPI, Grafos e Programaçao 2. Observando nosso modelo melhorado, chegamos a conclusão que as variáveis que foram removidas não influenciavam no modelo, pois o R² e o R² ajustado se mantiveram constantes, por isso, nosso modelo continua a explicar ~64% do CRA, em outras palavras, as disciplinas que estamos considerando explicam 64% do cra do aluno. 
