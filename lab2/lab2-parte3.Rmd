...
test.bruto <- read.csv("dados/teste_1_2_periodo.csv")
...
colnames(test.bruto) <- c("matricula", "ano_evasao", "periodo_evasao", "cod_disciplina", "disciplina", "creditos", "media")
```
...
#renomeando colunas
colnames(p1.p2.train) <- c("matricula", "C1", "Vetorial", "LPT", "P1", "IC", "LP1","C2", "Discreta", "P2", "Grafos", "Física", "LP2", "CRA")

#removendo NaN
p1.p2.train <- na.omit(p1.p2.train)
...
#renomeando colunas
colnames(p1.p2.test) <- c("matricula", "C1", "Vetorial", "LPT", "P1", "IC", "LP1","C2", "Discreta", "P2", "Grafos", "Física", "LP2", "CRA")
...
#ordenando os dados pela matrícula
test.bruto <- test.bruto %>%
  arrange(matricula)

#filtrando dados e removendo os NaNs
graduados.clean.test <- test.bruto %>%
  filter(!is.na(media))

#calculando CRA dos alunos e salvando numa coluna
graduados.cra.test <- graduados.clean.test %>%
  group_by(matricula) %>%
  mutate(cra.contrb = media*creditos) %>%
  summarise(cra = sum(cra.contrb)/sum(creditos))

#utilizando a função dcast para deixar o dataset na forma ideal para a análise
graduados.model.input2 <- graduados.clean.test %>%
  group_by(matricula, disciplina) %>%
  filter(media == max(media))%>%
  ungroup() %>%
  select(matricula, disciplina, media) %>%
  mutate(disciplina = as.factor(gsub(" ", ".", disciplina))) %>%
  dcast(matricula ~disciplina, mean) %>%
  merge(graduados.cra.test)

#selecionando cra e disciplinas do primeiro e segundo período
test.bruto <- graduados.model.input2 %>%
  select(matricula, Laboratório.de.Programação.I, Programação.I, Introdução.à.Computação, Cálculo.Diferencial.e.Integral.I, Álgebra.Vetorial.e.Geometria.Analítica, Leitura.e.Produção.de.Textos, Cálculo.Diferencial.e.Integral.II, Matemática.Discreta, Programação.II, Teoria.dos.Grafos, Fundamentos.de.Física.Clássica, Laboratório.de.Programação.II, cra)
...
#renomeando colunas
colnames(test.bruto) <- c("matricula", "C1", "Vetorial", "LPT", "P1", "IC", "LP1","C2", "Discreta", "P2", "Grafos", "Física", "LP2", "CRA")

test.bruto <- na.omit(test.bruto)
...
correlationMatrix <- cor(p1.p2.train %>% select(-matricula))
...
ridge <- train(CRA ~ . ,data = p1.p2.train %>% select(-matricula),
                        method = "ridge",
...
```{r}
ridge_prediction <- predict(ridge, p1.p2.test)
p1.p2.test <- na.omit(p1.p2.test)
ridge_prediction <- data.frame(pred = ridge_prediction, obs = p1.p2.test$CRA)
round(defaultSummary(ridge_prediction), digits = 3)
```
...
lasso <- train(CRA ~ . ,data = p1.p2.train %>% select(-matricula),
                        method = "lasso",
                        tuneLength = 10,
                        metric = 'RMSE',
                        preProcess=c('scale', 'center'))
...
svm <- train(CRA ~ . ,data = p1.p2.train %>% select(-matricula),
                        method = "svmRadial",
                        tuneLength = 10,
                        metric = 'RMSE')
plot(svm)

svm_prediction <- predict(svm, p1.p2.train)

svm_prediction <- data.frame(pred = svm_prediction, obs = p1.p2.train$CRA)
...
colnames(test) <- c("matricula", "C1", "Vetorial", "LPT", "P1", "IC", "LP1","C2", "Discreta", "P2", "Grafos", "Física", "LP2")

lasso_prediction_t <- predict(lasso, test)

submissao <- data.frame(matricula = test$matricula, 
                     cra = lasso_prediction_t)
write.csv(submissao, file = "sub1.csv", row.names = F)
...
```{r}
lm <- train(CRA ~ . ,p1.p2.train %>% select(-matricula), method= "lm", metric="RMSE") 
summary(lm)

lmFit <- train(CRA ~ ., p1.p2.train %>% select(-matricula, -C1, -LP2), method= "lm", metric="RMSE") 
summary(lmFit)
