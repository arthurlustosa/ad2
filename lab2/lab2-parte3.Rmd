...
```{r}
graduados.train <- read.csv("dados/graduados_treino_base.csv")
graduados.test <- read.csv("dados/graduados_treino_avaliacao.csv")
```

```{r}
colnames(graduados.train) <- c("matricula", "ano_evasao", "periodo_evasao", "cod_disciplina", "disciplina", "media")
```
...
graduados.cra <- graduados.clean %>%
  group_by(matricula) %>%
  mutate(cra.contrb = media*periodo_evasao) %>%
  summarise(cra = sum(cra.contrb)/sum(periodo_evasao))
